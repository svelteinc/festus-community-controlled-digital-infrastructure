...
def test_per_mw_fees_are_used_when_annual_values_zero() -> None:
    inputs = Phase1CapitalStackInputs(
        total_project_cost=50_000_000.0,
        senior_debt_ratio=0.5,
        annual_interest_rate=0.05,
        debt_tenor_years=10,
        annual_cash_flow_available_for_debt_service=8_000_000.0,
        annual_ground_rent=0.0,
        annual_usage_fee=0.0,
        capacity_mw=20.0,
    ...
    # 20 MW * (50k + 10k) = 1.2M of community revenue.
    assert outputs.annual_community_revenue == pytest.approx(1_200_000.0)


def test_annual_values_override_per_mw_when_both_provided() -> None:
    inputs = Phase1CapitalStackInputs(
        total_project_cost=50_000_000.0,
        senior_debt_ratio=0.5,
        annual_interest_rate=0.05,
    ...
    # Annual values should take precedence over per-MW values.
    assert outputs.annual_community_revenue == pytest.approx(1_500_000.0)
